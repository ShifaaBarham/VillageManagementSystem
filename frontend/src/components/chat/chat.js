...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    ws.current.onmessage = (event) => {
      ...
      setMessages((prevMessages) => [
        ...prevMessages,
        { text: `${receivedMessage.sender}: ${receivedMessage.text}`, isAdmin: receivedMessage.sender === 'admin' },
      ]);
    };

    ws.current.onclose = () => {
      console.log('تم غلق الاتصال بخادم WebSocket');
    };
  ...
  const openChat = (admin) => {
    setAdminName(admin);
    setChatVisible(true);
    setMessages([ { text: `${admin}: Hello! How can I assist you today?`, isAdmin: true } ]);
  };
  ...
  return (
    <section id="Chat" className="section">
      ...
      {chatVisible && (
        <section id="chat-section">
          <h3>Chat with {adminName}</h3>
          <div id="chat-window">
            {messages.map((message, index) => (
              <div key={index} style={{ color: message.isAdmin ? '#4A90E2' : '#34D399' }}>
                {message.text}
              </div>
            ))}
          </div>
