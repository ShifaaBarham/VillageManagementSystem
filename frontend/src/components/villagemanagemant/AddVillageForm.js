...
const AddVillageForm = ({ onClose, refreshVillages }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const landAreaValue = parseFloat(formData.land_area);
    ...
    if (isNaN(landAreaValue) || landAreaValue < 0) {
      alert("Land area must be a valid non-negative number.");
      return;
    }
    ...
    try {
      await addVillage({
        variables: {
          ...
          tags: parsedTags.length > 0 ? parsedTags : ["general"], // افتراض "general" إذا لم تُدخل أي وسوم
        },
      });
