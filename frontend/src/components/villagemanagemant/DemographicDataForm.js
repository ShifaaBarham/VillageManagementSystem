...
const DemographicDataForm = ({ villageId, currentData, onClose, refreshVillages }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    let parsedAgeDistribution;
    try {
      parsedAgeDistribution = JSON.parse(formData.ageDistribution);
    } catch {
      parsedAgeDistribution = formData.ageDistribution
        .split(",")
        .reduce((acc, item) => {
    ...
    let parsedGenderRatios;
    try {
      parsedGenderRatios = JSON.parse(formData.genderRatios);
    } catch {
      parsedGenderRatios = formData.genderRatios
        .split(",")
        .reduce((acc, item) => {
          const [key, value] = item.split(":").map((str) => str.trim());
          if (key && value) acc[key] = parseFloat(value);
