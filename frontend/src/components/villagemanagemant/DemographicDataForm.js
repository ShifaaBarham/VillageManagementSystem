...
const DemographicDataForm = ({ villageId, currentData, onClose, refreshVillages }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // تحليل المدخلات وتحويلها إلى JSON إذا كانت صالحة
    const parsedAgeDistribution = formData.ageDistribution.includes("{") && formData.ageDistribution.includes("}")
      ? JSON.parse(formData.ageDistribution)
      : formData.ageDistribution.split(",").reduce((acc, item) => {
          const [key, value] = item.split(":").map((str) => str.trim());
          if (key && value) acc[key] = parseFloat(value);
          return acc;
        }, {});
  
    const parsedGenderRatios = formData.genderRatios.includes("{") && formData.genderRatios.includes("}")
      ? JSON.parse(formData.genderRatios)
      : formData.genderRatios.split(",").reduce((acc, item) => {
          const [key, value] = item.split(":").map((str) => str.trim());
          if (key && value) acc[key] = parseFloat(value);
          return acc;
        }, {});
    ...
    try {
      await updateDemographicData({
        variables: {
          ...
          population_distribution: JSON.stringify(parsedAgeDistribution),
          gender_ratios: JSON.stringify(parsedGenderRatios),
